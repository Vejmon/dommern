...
const routeToKuskOrBack = async (kusk: Kusk) => {
  if (kusk.id === route.params.id) {
    await router.push({name: 'leaderBoard'})
  } else {
    await router.push({name: 'viewKusk', params: {id: kusk.id}})
  }
}
    ...
    <div class="flex flex-col xl:flex-row justify-center">
      <div class="xl:min-w-7xl min-w-sm flex flex-col gap-4">
        <div v-for="kusk in kusks" :key="kusk.id">
          <div :class="`bane-${kusk.currentBane} text-3xl hover:cursor-pointer`" @click.prevent="routeToKuskOrBack(kusk)">
